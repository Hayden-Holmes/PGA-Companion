...
Design:
  ...
  - DB writes are committed per logical unit (tournament, scorecard player).
"""
...
# Seconds to sleep between scorecard stat fetches to avoid rate limiting.
...
class Pipeline:
    ...
    # ------------------------------------------------------------------
    # Step 3: Process one tournament end-to-end
    # ------------------------------------------------------------------
    ...
    def process_tournament(
        ...
        """
        For one tournament:
          1. Normalize and upsert tournament row.
          2. Normalize and upsert courses (from details).
          3. Fetch courseStats to fill par/yardage.
          4. Skip leaderboard if not COMPLETED.
          5. Fetch (or load from disk) and decode leaderboard.
          6. Normalize and upsert players and rounds (scores only).
          7. Fetch scorecard stats per player; upsert SG/performance
             columns onto the round rows inserted in step 6.
        """
        tid    = schedule_row["id"]
        ...
        # Build location string once — used for courses throughout
        ...
        # --- Course rows from details ---
        course_rows = []
        if details_row:
            for c in details_row.get("courses") or []:
                course_rows.append(normalize_course(c, location))
        else:
            logger.warning(
                "tournament_id=%s: no details available, course row will be incomplete", tid
            )
        ...
        # --- Course stats: fill par and yardage ---
        ...
        if decoded_lb is None:
            ...
            if decoded_lb is None:
                logger.warning("No leaderboard available for completed tournament %s", tid)
                return
            self.store.save_leaderboard(tid, decoded_lb)
        else:
            logger.info("Loaded leaderboard from disk for %s", tid)

        # Guard: season_year must be a valid int before we proceed
        try:
            season_year_int = int(season_year)
        except (ValueError, TypeError):
            logger.error(
                "tournament_id=%s: invalid season_year %r, skipping leaderboard load",
                tid, season_year,
            )
            return
        ...
        logger.info(
            "Loaded %s: %d players, %d rounds",
            tid, len(players), len(rounds),
        )

        # --- Scorecard stats: one call per player ---
        # Rounds are now in the DB. Fetch SG splits and performance stats
        # for each player and upsert them onto the existing round rows.
        # A failure for one player is logged and skipped.
        if not players:
            logger.info("%s: no players found, skipping scorecard stats", tid)
            return

        logger.info("%s: fetching scorecard stats for %d players", tid, len(players))
    ...
    def run_year(self, year: str, completed_only: bool = True):
        ...
        # Fetch details in batched chunks (50 is conservative for API limits)
        details_by_id = {}
        chunk_size = 50
        for i in range(0, len(all_ids), chunk_size):
            chunk = all_ids[i : i + chunk_size]
            details_by_id.update(self.fetch_and_store_details(chunk))
        ...
        for sched_row in schedule:
            ...
            sy          = details_row.get("seasonYear", year) if details_row else year
        ...
        if not stat_details_list:
            return

        stat_rows = extract_season_stats(stat_details_list)

        # Upsert any new players found in stat data
        stat_players = {}
        ...
        if stat_rows:
            ...
            logger.info("Loaded %d season stat rows for year=%s", len(stat_rows), year)
